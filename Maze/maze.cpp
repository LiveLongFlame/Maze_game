...
vector<vector<char>> Maze::generate_maze(int width, int height) {
	...
	for (int i = 1; i < width; i += 2) {
		if (maze_desgin[1][i] == ' ')
			edge_cells.emplace_back(i, 0); // Top edge
		if (maze_desgin[height - 2][i] == ' ')
			edge_cells.emplace_back(i, height - 1); // Bottom edge
	}

	for (int i = 1; i < height; i += 2) {
		if (maze_desgin[i][1] == ' ')
			edge_cells.emplace_back(0, i); // Left edge
		if (maze_desgin[i][width - 2] == ' ')
			edge_cells.emplace_back(width - 1, i); // Right edge
	}

	if (edge_cells.size() < 2)
		return maze_desgin; // Can't place start/end if less than 2 exits

	// Pick random start and end from edge_cells
	int si = rand() % edge_cells.size();
	int ei = rand() % edge_cells.size();
	while (ei == si)
		ei = rand() % edge_cells.size(); // Ensure different positions
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			...
			if (start_y + i >= 0 && start_y + i < term_rows &&
				...
				bool isPlayer = player_visited_path[i][j];
				bool isAlgo = algo_show && algo_path[i][j];
